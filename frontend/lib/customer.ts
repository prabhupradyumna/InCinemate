...
export async function getBookingDetails(id: string): Promise<BookingWithDetails> {
...
export async function holdSeats(payload: HoldSeatsPayload): Promise<SeatHoldResult> {
...
export async function applyCoupon(bookingId: string, couponId: string): Promise<{
  booking_id: string;
...
export async function initiateRefund(bookingId: string, reason?: string): Promise<{
  success: boolean;
  refund_id?: string;
  status: string;
  message: string;
  ...
  const res = await api.post(`/customer/bookings/${bookingId}/refund`, { reason });
  return res.data?.data;
}
...
export async function downloadTicket(bookingId: string, format: 'pdf' | 'image' = 'pdf'): Promise<Blob> {
...
export async function addFavoriteMovie(movieId: string): Promise<{
  ...
  const res = await api.post('/customer/favorites/movies', { movie_id: movieId });
  return res.data?.data;
}
...
export async function addFavoriteTheatre(theatreId: string): Promise<{
  ...
  const res = await api.post('/customer/favorites/theatres', { theatre_id: theatreId });
  return res.data?.data;
}
...
export async function replyToSupportTicket(ticketId: string, message: string): Promise<{
  success: boolean;
  message: string;
}> {
