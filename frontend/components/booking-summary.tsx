...
export function BookingSummary({ showData, selectedSeats = [] }: BookingSummaryProps) {
  const [step, setStep] = useState<"summary" | "details" | "payment">("summary")
  ...
  // Mock selected seats for demo - in real app this would be passed from parent
  const mockSelectedSeats =
    selectedSeats.length > 0
      ? selectedSeats
      : [
          { row: "A", seat: 1, type: "premium" as const },
          { row: "A", seat: 2, type: "premium" as const },
        ]
      ...
      <Card className="bg-card border-border sticky top-24">
        ...
        <CardContent className="space-y-3 md:space-y-4">
              ...
              <div className="space-y-1">
                {mockSelectedSeats.map((seat, index) => (
                  <div key={index} className="flex justify-between text-xs md:text-sm">
                    ...
                    <span>${showData.showtime.pricing[seat.type].toFixed(2)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-2 pt-4">
            ...
            {step === "details" && (
              <>
                <Button className="w-full cinema-glow" onClick={handleContinue} disabled={!isDetailsValid}>
                  Continue to Payment
                </Button>
